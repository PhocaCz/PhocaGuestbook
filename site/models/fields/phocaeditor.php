...
class JFormFieldPhocaEditor extends JFormField
{
	...
	protected function getInput(){
		...
		if ($editor) {
            // We work only with tinymce

            $plugin 	= PluginHelper::getPlugin('editors', 'tinymce');
			$paramsE 	= new Registry($plugin->params);
            ...
            $id            = preg_replace('/(\s|[^A-Za-z0-9_])+/', '_', $this->id);
            $nameGroup     = explode('[', preg_replace('/\[\]|\]/', '', $this->name));
            ...
            if (is_numeric($this->width)) {$this->width .= 'px';}
            if (is_numeric($this->height)) {$this->height .= 'px';}
           ...
           // }
            ...
            // Only public and guest options for tinymce
            $ugroups = array(1  => 1, 9 => 9);
            ...
            $extraOptionsAll  = (array) $paramsE->get('configuration.setoptions', array());
            $toolbarParamsAll = (array) $paramsE->get('configuration.toolbars', array());
           ...
           /* $db    = Factory::getDbo();
            ...
            $html_height = $paramsE->get('html_height', '550');
            $html_width  = $paramsE->get('html_width', '');
          ...
          /* $plugins  = array(
           ...
           /* foreach (array('wordcount' => 1, 'advlist' => 1, 'autosave' => 1, 'textpattern' => 0) as $pName => $def)
            ...
            if ($dragdrop && $user->authorise('core.create', 'com_media'))
            {
                ...
                // @TO DO have a way to select the adapter, similar to $levelParams->get('path', '');
...
/*
    private function tinyButtons($name, $excluded)
	{
		...
		$buttonsResult = $app->getDispatcher()->dispatch('getButtons', $buttonsEvent);
